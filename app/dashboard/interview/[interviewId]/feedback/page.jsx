...
function Feedback({ params: paramsPromise }) {
  ...
  const fetchFeedback = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/answers?mockId=${params.interviewId}`);
  ...
  const calculateAverageRating = (feedbackList) => {
    ...
    setAverageRating((totalRating / feedbackList.length).toFixed(1));
  };
    ...
    <div className="min-h-screen bg-black p-4 md:p-10">
      <div className="max-w-4xl mx-auto">
        <div className="bg-gradient-to-br from-cyan-900 to-cyan-700/90 text-white rounded-xl shadow-lg p-6 mb-6 backdrop-blur-md border border-cyan-500/50">
          <h2 className="text-2xl font-extrabold text-white mb-2 text-center">🎉 Congratulations!</h2>
          <h3 className="text-lg font-medium text-gray-300 mb-4 text-center">Here's your interview feedback</h3>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {feedbackList.map((item, index) => (
              <Collapsible key={item._id} className="bg-gray-900 rounded-lg shadow-md overflow-hidden border border-gray-700">
                <CollapsibleTrigger className="w-full p-4 hover:bg-cyan-800/20 transition-all duration-200">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span className="bg-cyan-500 text-black px-3 py-1 rounded-full font-semibold shadow-md">Q{index + 1}</span>
        ...
        <div className="mt-8 flex gap-4 justify-center">
          ...
          <Dialog>
            ...
            <DialogContent className="max-w-2xl bg-gray-900 text-gray-200 border border-cyan-500 shadow-lg rounded-lg">
              <DialogHeader>
                <DialogTitle className="text-xl font-bold text-cyan-300">Performance Analysis</DialogTitle>
              </DialogHeader>
